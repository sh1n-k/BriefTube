...
def load_config() -> AppConfig:
    ...
    cfg = AppConfig(
        ...
        download_max_concurrent=int(
            file_values.get("download_max_concurrent", base.download_max_concurrent)
        ),
        download_timeout_seconds=int(
            file_values.get("download_timeout_seconds", base.download_timeout_seconds)
        ),
        ...
        rss_consecutive_error_abort_threshold=int(
            file_values.get(
                "rss_consecutive_error_abort_threshold", base.rss_consecutive_error_abort_threshold
            )
        ),
        ...
        log_dependency_level=str(
            file_values.get("log_dependency_level", base.log_dependency_level)
        ),
    )
    ...
    cfg.download_max_concurrent = int(
        os.getenv("DOWNLOAD_MAX_CONCURRENT", cfg.download_max_concurrent)
    )
    cfg.download_timeout_seconds = int(
        os.getenv("DOWNLOAD_TIMEOUT_SECONDS", cfg.download_timeout_seconds)
    )
    ...
    cfg.rss_consecutive_error_abort_threshold = int(
        os.getenv(
            "RSS_CONSECUTIVE_ERROR_ABORT_THRESHOLD", cfg.rss_consecutive_error_abort_threshold
        )
    )
    ...
    cfg.transcript_channel_min_interval_seconds = max(
        0, cfg.transcript_channel_min_interval_seconds
    )
    cfg.transcript_channel_pick_lookahead = max(1, cfg.transcript_channel_pick_lookahead)
    cfg.transcript_channel_hard_cooldown_seconds = max(
        1, cfg.transcript_channel_hard_cooldown_seconds
    )
